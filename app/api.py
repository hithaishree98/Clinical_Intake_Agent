...
@app.post("/chat")
def chat(thread_id: str = Form(...), message: str = Form(...), client_msg_id: str = Form(...)):
    ...
    try:
        ...
        resp_obj = {"reply": reply, "status": status, "phase": output.get("current_phase")}
        db.save_idempotent_response(thread_id, client_msg_id, resp_obj)
...
def clinician_pending():
    return JSONResponse(content=db.list_pending_escalations())

@app.post("/clinician/resolve")
def clinician_resolve(thread_id: str = Form(...), esc_id: str = Form(...), nurse_note: str = Form("Resolved")):
