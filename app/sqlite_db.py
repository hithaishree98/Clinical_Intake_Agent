...
def get_idempotent_response(thread_id: str, key: str):
    return fetch_one("SELECT response_json FROM idempotency WHERE thread_id=? AND key=?", (thread_id, key))

def save_idempotent_response(thread_id: str, key: str, response_obj: dict):
    exec_one(
        "INSERT OR REPLACE INTO idempotency(thread_id, key, response_json) VALUES (?,?,?)",
        (thread_id, key, json.dumps(response_obj)),
    )
